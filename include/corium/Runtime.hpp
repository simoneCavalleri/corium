...
public:
    ...
    using TimerSchedulerType = TimerScheduler<EventVariant, TimerStoragePolicy::max_timers>;
    ...
    {
        return _timerScheduler.scheduleDelayed(std::move(event), std::chrono::duration_cast<std::chrono::microseconds>(delay), priority);
    }
    ...
    {
        return _timerScheduler.schedulePeriodic(std::move(event), std::chrono::duration_cast<std::chrono::microseconds>(interval), priority);
    }
    ...
    {
        auto ctx = AppCoreContextT<EventBusType>{
            _eventBus,
            StaticCallback{
...
/// @brief Default Runtime alias using DefaultEvents, NoSignalPolicy, DefaultStoragePolicy, DropNewestOverflowPolicy, and DefaultTimerStoragePolicy.
using Runtime = BasicRuntime<DefaultEvents, BoundedMpscQueuePolicy<DefaultEvents, 1024>, NoSignalPolicy, DefaultStoragePolicy, DropNewestOverflowPolicy, DefaultTimerStoragePolicy>;
...
template <
    ...
    typename TimerStoragePolicy = DefaultTimerStoragePolicy
>
using RuntimeT = BasicRuntime<EventVariant, QueuePolicy, SignalPolicy, StoragePolicy, OverflowPolicy, TimerStoragePolicy>;
