...
TEST(ReactorTest, SealedReactorState) {
    Reactor reactor;
    EXPECT_FALSE(reactor.sealed());
    reactor.seal();
    EXPECT_TRUE(reactor.sealed());

#ifndef NDEBUG
    EXPECT_DEATH(reactor.registerHandler([](const TickEvent&) {}), "registerHandler.*sealed");
#endif
}
