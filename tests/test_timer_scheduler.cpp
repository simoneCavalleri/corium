...
TEST(TimerSchedulerTest, RuntimeIntegrationWithAppCore)
{
    ...
    class App : public AppCoreT<App, TimerRuntime::EventBusType> {
    public:
        int alertCount = 0;
        int tickCount = 0;

        void onRegisterHandlers() {
            on([this](const ScheduledAlertEvent&) {
                alertCount++;
            });

            on([this](const TimerTickEvent&) {
                tickCount++;
            });
        }
    };
