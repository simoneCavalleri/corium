...
namespace {
class DummyService1 : public ProducerService<> {};
class DummyService2 : public ProducerService<> {};

class MultiServiceApp : public Application<MultiServiceApp, Runtime::EventBusType, 2> {
public:
    DummyService1 s1;
    DummyService2 s2;
    DummyService1 s3;
    ...
    void onConfigureServices(Registry& reg) {
        EXPECT_TRUE(reg.registerService(s1));
        EXPECT_TRUE(reg.registerService(s2));
        // 3rd registration exceeds MaxServices=2
        EXPECT_FALSE(reg.registerService(s3));
        overflowFailed = true;
    }
};
} // namespace

TEST(RuntimeTest, CustomMaxServicesApplication) {
    Runtime runtime;
    MultiServiceApp app;
    runtime.initialize(app);
    EXPECT_TRUE(app.overflowFailed);
    runtime.shutdown();
}
